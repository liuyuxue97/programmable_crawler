...
from elasticsearch_dsl import Search, Index, DocType
...
def openCrawlerConfig(webSiteEntity):
    crawlerConfig = open(os.getcwd()+'.\ENVRI-crawlerConfig.json',"r")
...
def ingest_metadataFile(metadataFile):
    ...
    if not es.indices.exists(index=config['decision_model']):
        ...
        res = es.index(index=config['decision_model'], id=id, body=metadataFile,doc_type='_doc')
...
def getByInfix(feature,tag):
    global config
    preTag = tag.find_previous(config['features'][feature]['searchKeywords']['prefix']['tag'],
                               class_=config['features'][feature]['searchKeywords']['prefix']['cssClass'])
    if type(tag)!=type(None):
